...
export async function createMoment({ mediaUrl, type, durationMs, metadata }) {
  const res = await axiosInstance.post("/moments", { mediaUrl, type, durationMs, metadata });
  return res.data;
}
...
export async function truncateChannel(channelId, type = "messaging") {
  const res = await axiosInstance.post("/chat/truncate", { channelId, type });
  return res.data;
}