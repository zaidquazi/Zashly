        ...
        <ul className="flex flex-col divide-y divide-base-200">
          {items.map((u, i) => (
            <li key={u.channelId || u.id || i}>
              <div
                className={`relative flex items-center gap-3 px-2 sm:px-1 hover:bg-base-200/40 transition-colors cursor-pointer min-h-[64px] sm:min-h-0 ${
                  compact ? "py-2" : "py-3"
                }`}
              ...
              >
                <ProfileAvatar
                  src={u.avatar}
                  name={u.name}
                  size={compact ? "w-10 h-10 sm:w-9 sm:h-9" : "w-14 h-14 sm:w-12 sm:h-12"}
